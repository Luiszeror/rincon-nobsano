...
/*
  Anuncios — soporta 3 tipos de medio:
  - video: { tipo: 'video', video: '/ruta/video.mp4', titulo, texto, fecha }
      Se muestra más grande que las fotos, con su descripción debajo.
  - foto:  { tipo: 'foto', imagen: '/ruta/foto.jpg', titulo, texto, fecha }
...
export function Anuncios({ items }) {
  return (
    <div>
      {items.map((a, i) => {
        const esVideo = a.tipo === 'video' && a.video
        return (
          <div key={i} className={`anuncio-card ${esVideo ? 'anuncio-card--video' : ''}`}>
            <div className={`anuncio-media ${esVideo ? 'anuncio-media--video' : ''}`}>
              {esVideo ? (
                <video
                  src={a.video}
                  controls
                  poster={a.poster || '/cafe_pred.jpg'}
            ...
            <div>
              <span className="anuncio-fecha">{a.fecha}</span>
              <h3>{a.titulo}</h3>
              <p>{a.texto}</p>
            </div>
          </div>
...
export function Ubicacion({ titulo, descripcion, direccion, mapsQuery }) {
  const embed = `https://maps.google.com/maps?q=${encodeURIComponent(mapsQuery)}&z=15&output=embed`
  const link = `https://maps.google.com/?q=${encodeURIComponent(mapsQuery)}`
  return (
    <div className="ubicacion-box">
      <div className="ubicacion-info">
        <h3>{titulo}</h3>
        <p>{descripcion}</p>
        <p style={{ marginTop: 14, fontWeight: 700 }}>{direccion}</p>
        <a className="btn-maps" href={link} target="_blank" rel="noopener noreferrer">
